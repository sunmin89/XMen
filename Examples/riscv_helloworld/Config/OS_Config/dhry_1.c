...
//   define DHRY_ITERS 8000000
#define DHRY_ITERS 20000000

#endif

#define RANDOM_ITERS false

#define RICH_INFO false
...
static unsigned long seed = 1; // 随机数种子

// 自定义 rand 函数
int my_rand(void) {
    // 线性同余生成器公式：Xn+1 = (a * Xn + c) % m
    seed = (1103515245 * seed + 12345) % (RAND_MAX_VALUE + 1);
    return (int)seed;
}
  ...
  // 设置种子
    ...
    if(RANDOM_ITERS){
      for (int i = 0; i < 100; i++) {
        ...
        dhrystone(DHRY_ITERS);
    }
    // printf("aaaaaa           %d\n",iter);
    // iter =0;
}
 
 int dhrystone (int iter)
 /*****/
...
// uint32
tick = Os_CCB_Inf[Os_SCB.sysCore]->counterCurVal;
// printf ("        tick  == :   %f\n", (float)tick);
...
tok = Os_CCB_Inf[Os_SCB.sysCore]->counterCurVal;
// printf ("        tok   == :   %f\n", (float)tok);
// printf ("        delta(int)    == :   %d\n", (tok - tick));
float delta = (float)OS_TICKS2SEC_SystemTimer_Core0(tok - tick);
...
//  myprintf("-----------------------------------------------------------------------------------\n");
//  myprintf("-----------------------------------------------------------------------------------\n");
// printf ("----- DHRY_ITERS = : %d,        Elapse = :   %f Seconds -----\n", iter, delta);
//  myprintf("-----------------------------------------------------------------------------------\n");
//  myprintf("-----------------------------------------------------------------------------------\n");
//  myprintf("-----------------------------------------------------------------------------------\n");
//  myprintf("-----------------------------------------------------------------------------------\n");
if(RICH_INFO){
  ...
  memfree(Next_Ptr_Glob);
  memfree(Ptr_Glob);

  int deltaint = tok - tick;
